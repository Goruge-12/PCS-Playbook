...
function normalizeBase(base) {
  return {
    ...
    major_units: base.major_units || base.unit_contact_info || 'Units managed by admin.',
  };
}
...
function Installations() {
  ...
  async function loadInstallations() {
    try {
      const res = await api.get(`/installations?search=${encodeURIComponent(search)}`);
      if (res.data?.length) setInstallations(res.data.map(normalizeBase));
      else setInstallations(filterDemo());
    } catch {
  ...
  function filterDemo() {
    ...
    return demoInstallations.filter((base) =>
      `${base.installation_name} ${base.city} ${base.state} ${base.zip_code}`.toLowerCase().includes(keyword)
    );
  }
      ...
      <div className="map-card">
        <MapContainer center={[36.5, -96]} zoom={4} className="map">
          ...
          {installations.map((base) => (
            <Marker key={base.slug} position={[Number(base.latitude), Number(base.longitude)]}>
              <Popup>
                <strong>{base.installation_name}</strong><br />
                {base.city}<br />
                <Link to={`/installations/${base.slug}`}>View Installation</Link>
              </Popup>
            </Marker>
          ))}
        </MapContainer>
  ...
  />
            ...
            <p><strong>Major Units:</strong> {base.major_units}</p>
            <Link className="button" to={`/installations/${base.slug}`}>View Installation</Link>
          </div>
        ))}
      </div>
    </section>
